...
def load_model(model: str, protocol: str, n_features: int, epoch_to_rec_at: int, rec_alpha: float, rec_lambda: float, rec_keep_prob: float, regularization: str):
    if model == "FM" and protocol == "A":
        model = FM(
            n_users=n_users, 
            n_items=n_items,
